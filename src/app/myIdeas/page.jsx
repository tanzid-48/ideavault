...
const MyIdeasPage = async () => {
  const userEmail = "tanzid@gmail.com"; // TODO: auth
  const ideas = await getMyIdeas(userEmail);

  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-zinc-800 pb-5">
        <div>
          <h1 className="text-2xl font-black text-gray-900 dark:text-white">My Ideas</h1>
      ...
      {ideas.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {ideas.map((idea) => <MyIdeaCard key={idea._id} idea={idea} />)}
        </div>
      )}
    </div>
  );
